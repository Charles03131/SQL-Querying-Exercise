...
2.SELECT id,app_name FROM analytics WHERE last_updated='08-01-2018';
...
7.SELECT app_name, price, rating FROM analytics WHERE price<3 ORDER BY price DESC LIMIT 1;
...
9.SELECT app_name FROM analytics WHERE ratings<3 AND reviews>=10000;
...
--woulnt we use COUNT instead of SUM? we just want the total number of reviews ? not the sum of values of the reviews?==
13.SELECT COUNT(reviews) AS "All The Reviews" FROM analytics;
...
15. SELECT app_name,reviews,min_install, min_install/reviews AS proportion FROM analytics WHERE min_installs>=100000 ORDER BY proportion DESC LIMIT 1;